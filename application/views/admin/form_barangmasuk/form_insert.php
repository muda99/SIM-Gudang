      ...
      <ul class="sidebar-menu" data-widget="tree">
        ...
        <li class="treeview active">
          ...
          <ul class="treeview-menu">
            ...
            <li><a href="<?= base_url('admin/form_barang')?>"><i class="fa fa-circle-o"></i> Tambah Barang</a></li>
          </ul>
        </li>
        <li class="treeview ">
          <a href="#">
    ...
    <section class="content">
      <div class="row">
        ...
        <!--         <div class="form-group" style="display:inline-block;">
                  <label for="kode_barang" style="width:80%;margin-left:0px;">Kode Barang / Barcode</label>
                  <input type="text" name="kode_barang" style="width: 80%;margin-left: 5px;" class="form-control" class="form-control" readonly="readonly" value="UT-<?=date("Y");?><?=random_string('numeric', 8);?>">
                </div> -->

<div class="form-group" style="display:inline-block;">
    <label for="kode_barang" style="width:80%;margin-left:0px;">Kode Barang / Barcode</label>
    <input type="text" name="kode_barang" style="width: 80%;margin-left: 5px;" class="form-control" readonly="readonly">
</div>
<!-- <div class="form-group" style="display:inline-block;">
    <label for="unit" style="width:73%;margin-left:-15px;">Unit</label>
    <select id="unit_dropdown" class="form-control" name="unit" style="width:110%;margin-right: 18px;margin-left:-15px;">
        <option value="" selected="">-- Pilih --</option>
        <?php foreach($list_unit as $s): ?>
            <option value="<?= $s->id_unit ?>"><?= $s->nama_unit ?></option>
        <?php endforeach; ?>
    </select>
</div> -->
<div class="form-group" style="display:inline-block;">
    <label for="barang" style="width:73%;margin-left:20px;">Nama Barang</label>
    <select id="barang_dropdown" class="form-control" name="nama_barang" style="width:90%;margin-right: 67px;margin-left:20px;">
        <option value="" selected>-- Pilih --</option>
        <?php foreach($list_barang as $s): ?>
            <option value="<?= $s->nama_barang ?>"data-id="<?= $s->kode_barang ?>"><?= $s->nama_barang ?></option>
        <?php endforeach; ?>
    </select>
</div>

<script>
    document.getElementById('barang_dropdown').addEventListener('change', function() {
        var selectedOption = this.options[this.selectedIndex];
        var kodeBarang = selectedOption.getAttribute('data-id');;
        var namaBarang = selectedOption.textContent;
        document.querySelector('input[name="kode_barang"]').value = kodeBarang;
         document.querySelector('input[name="nama_barang"]').value = namaBarang;
    });
</script>


                <div class="form-group" style="display:inline-block;">
                  <label for="satuan" style="width:73%;margin-left:10px;">Satuan</label>
                  <select class="form-control" name="satuan" style="width:110%;margin-right: 18px;margin-left:10px;">
                    <option value="" selected="">-- Pilih --</option>
...
<!--                 <div class="form-group" style="display:inline-block;">
                  <label for="supplier" style="width:73%;margin-left:25px;">Supplier</label>
                  <select class="form-control" name="supplier" style="width:110%;margin-right: 18px;margin-left:25px;">
                    <option value="" selected="">-- Pilih --</option>
 ...
 -->
              <div class="form-group" style="display:inline-block;">
                <label for="jumlah" style="width:73%;margin-left:45px;">Jumlah</label>
                <input type="number" name="jumlah" style="width:41%;margin-left:45px;margin-right:18px;" class="form-control" id="jumlah">
            </div>
            <div class="form-group" style="display:inline-block;">
              <button type="reset" class="btn btn-basic" name="btn_reset" style="width:65px;margin-left:-100px;"><i class="fa fa-eraser" aria-hidden="true"></i> </button>
            </div>
              <!-- /.box-body -->
  ...
  <script>
    document.getElementById('unit_dropdown').addEventListener('change', function() {
        var unitId = this.value;
        var barangDropdown = document.getElementById('barang_dropdown');
        // Kosongkan dropdown barang
        barangDropdown.innerHTML = '<option value="">-- Pilih --</option>';
        // Kirim permintaan AJAX untuk mendapatkan barang sesuai unit yang dipilih
        if (unitId !== '') {
            var xhr = new XMLHttpRequest();
            xhr.onreadystatechange = function() {
                if (xhr.readyState === XMLHttpRequest.DONE) {
                    if (xhr.status === 200) {
                        var barangData = JSON.parse(xhr.responseText);
                        // Tambahkan opsi barang sesuai dengan data yang diterima
          ...
          xhr.open('GET', '<?= site_url('admin/get_barang_by_unit')?>?unit_id=' + unitId, true);
